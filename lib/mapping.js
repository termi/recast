...
function Mapping(sourceLines, sourceLoc, targetLoc) {
    ...
    if (targetLoc) {
        SourceLocation.assert(targetLoc);
    } else {
...
function skipChars(
    ...
    if (targetComparison < 0) {
        ...
        var sourceCursor = sourceLines.firstNonSpacePos(sourceFromPos);
        ...
        while (comparePos(targetCursor, targetToPos) < 0 &&
            ...
            assert.ok(sourceLines.nextPos(sourceCursor));

            if (targetLines.firstNonSpacePos(targetCursor, true)) {
                assert.strictEqual(
                    sourceLines.firstNonSpacePos(sourceCursor, true),
                    sourceCursor
                );
        ...
        var sourceCursor = sourceLines.lastNonSpacePos(sourceFromPos);
        ...
        while (comparePos(targetToPos, targetCursor) < 0 &&
            ...
            assert.ok(sourceLines.prevPos(sourceCursor));

            if (targetLines.lastNonSpacePos(targetCursor, true)) {
                assert.strictEqual(
                    sourceLines.lastNonSpacePos(sourceCursor, true),
                    sourceCursor
                );
