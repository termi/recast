...
// specified).
Lp.firstNonSpacePos = function(startPos, modifyInPlace) {
    var lines = this;
    var pos = startPos ? (modifyInPlace ? startPos : {
        line: startPos.line,
...
Lp.lastNonSpacePos = function(endPos, modifyInPlace) {
    var lines = this;
    var pos = endPos ? (modifyInPlace ? endPos : {
        line: endPos.line,
...
Lp.skipChars = function(fromPos, targetLines, targetFromPos, targetToPos) {
    assert.ok(targetLines instanceof Lines);
    Position.assert(fromPos);
    ...
    var sourceInfos = getSecret(this).infos;
    var targetInfos = getSecret(targetLines).infos;

    if (targetComparison <= 0) {
        // Skipping forward.

        var sourceCursor = this.firstNonSpacePos(fromPos);
        var targetCursor = targetLines.firstNonSpacePos(targetFromPos);

        var sourceInfo = sourceInfos[sourceCursor.line - 1];
        var targetInfo = targetInfos[targetCursor.line - 1];

        while (targetCursor.line < targetToPos.line) {
            assert.strictEqual(
                sourceInfo.line.slice(
                    sourceInfo.sliceStart,
                    sourceInfo.sliceEnd
                ).replace(/\s+/g, ""),
                targetInfo.line.slice(
                    targetInfo.sliceStart,
                    targetInfo.sliceEnd
                ).replace(/\s+/g, "")
            );
            ...
            sourceInfo = sourceInfos[sourceCursor.line - 1];
            targetInfo = targetInfos[targetCursor.line - 1];
        }

        while (comparePos(targetCursor, targetToPos) < 0 &&
               targetLines.firstNonSpacePos(targetCursor, true)) {
            assert.strictEqual(
                this.firstNonSpacePos(sourceCursor, true),
                sourceCursor
            );

            assert.strictEqual(
                this.charAt(sourceCursor),
                targetLines.charAt(targetCursor)
            );

            if (comparePos(targetCursor, targetToPos) < 0 &&
                targetLines.nextPos(targetCursor)) {
                assert.ok(this.nextPos(sourceCursor));
            } else {
                break;
            }
        }
        ...
        var sourceCursor = this.lastNonSpacePos(fromPos);
        var targetCursor = targetLines.lastNonSpacePos(targetFromPos);
        ...
        while (targetCursor.line > targetToPos.line) {
            ...
            sourceCursor.line -= 1;
            sourceCursor.column = this.getLineLength(sourceCursor.line);
        ...
        while (comparePos(targetToPos, targetCursor) < 0 &&
               targetLines.lastNonSpacePos(targetCursor, true)) {
            assert.strictEqual(
                this.lastNonSpacePos(sourceCursor, true),
                sourceCursor
            );
            ...
            if (comparePos(targetToPos, targetCursor) < 0 &&
                targetLines.prevPos(targetCursor)) {
                assert.ok(this.prevPos(sourceCursor));
            } else {
                break;
            }
        }
